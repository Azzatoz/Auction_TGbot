...
def create_auction_message(lot):
    ...
    message = (
        ...
        f"Текущая ставка: {lot.get('start_price', 'Нет информации')}Р\n"
...
@bot.callback_query_handler(func=lambda call: call.data.startswith("timer_") or call.data == "info" or call.data.startswith("open_lot_"))
...
def send_welcome(message):
    ...
    if len(args) > 1:
        ...
        if response.status_code == 200:
            ...
            info_button = types.InlineKeyboardButton("ℹ️ Инфо", callback_data="info")
            bid_button = types.InlineKeyboardButton("Сделать ставку", callback_data=f"bid_{lot_id}")
            hidden_bid_button = types.InlineKeyboardButton("Настроить скрытую ставку", callback_data=f"hidden_bid_{lot_id}")
            media_button = types.InlineKeyboardButton("Смотреть фото/видео", callback_data=f"media_{lot_id}")
            custom_price_button = types.InlineKeyboardButton("Предложить свою цену", callback_data=f"custom_price_{lot_id}")
...
def place_bid(call):
    _, lot_id, user_id = call.data.split('_')
    response = requests.post(f'http://localhost:8000/lots/{lot_id}/bid/', json={"user_id": user_id})
    if response.status_code == 200:
        lot = response.json()
        lot_message = create_auction_message(lot)
        markup = types.InlineKeyboardMarkup()
        ...
        markup.add(custom_price_button)
        markup.add(back_button)
        bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text=lot_message, reply_markup=markup)
    else:
...
def send_main_menu(message):
    ...
    giveaway_button = types.InlineKeyboardButton("РОЗЫГРЫШ", callback_data="giveaway")
    leaderboard_button = types.InlineKeyboardButton("Таблица лидеров", callback_data="leaderboard")
    markup.row(my_lots_button)
    markup.row(rules_button, help_button)
    markup.row(giveaway_button, leaderboard_button)
